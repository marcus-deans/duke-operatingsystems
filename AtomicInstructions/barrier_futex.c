...
//uint32_t* FUTE = 12345;
//volatile std::atomic<int> FUTE = 12345;
/* 
...
void wait_at_barrier(int iteration) {
    ...
    if(local_thread_count == threadnum){
        atomic_store(&threads_at_barrier, 0);
        //assert(threads_at_barrier == 0);
        atomic_fetch_add(&barrier_opening_count, 1);
        //assert(local_open_count + 1 == barrier_opening_count);
        assert(local_thread_count == threadnum);
        //futex_wake(&FUTE, threadnum);
        futex_wake(&barrier_opening_count, threadnum);
    }
    else{
        while(1){
            //futex_wait(&FUTE, FUTE);
            futex_wait(&barrier_opening_count, iteration);
            //wait(100);
            local_open_count = atomic_load(&barrier_opening_count);
            if(local_open_count > iteration){
                futex_wake(&barrier_opening_count, threadnum);
                return;
            }
        }
    ...
    local_thread_count = atomic_load(&threads_at_barrier);
